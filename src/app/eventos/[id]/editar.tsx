...
export default function EditarEventoScreen() {
      ...
      try {
        ...
        if (eventoEncontrado) {
          ...
          if (eventoEncontrado.local && eventoEncontrado.local.coordinates) {
            const lng = eventoEncontrado.local.coordinates[0].toString();
            const lat = eventoEncontrado.local.coordinates[1].toString();
            setLongitude(lng);
            setLatitude(lat);
            setMapRegion({
  ...
  const handleSalvarAlteracoes = async () => {
    ...
    const rawData = {
      ...
      data: dataEvento,
      imagem: imagemBase64 || imagemUri || undefined,
      local: {
        type: 'Point',
    ...
    try {
      ...
      Alert.alert('Erro na atualização', apiMessage);
    } finally {
      setIsSaving(false);
    }
  };
